...
import BalanceDisplay from "../../components/addmoney/BalanceDisplay";
import DepositQuickAmounts from "../../components/addmoney/DepositQuickAmounts";
import { depositInfoCards } from "../../data/addMoneyData";
  ...
  const handleAddMoney = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/transaction/deposit/${account.id}`,
        {
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <main className="max-w-2xl mx-auto px-4 sm:px-6 py-10 space-y-6">
          ...
          {!fetchingAccount && account && <BalanceDisplay account={account} />}
          ...
          <div className="bg-[#0b0f19]/90 backdrop-blur-xl border border-white/5 shadow-2xl p-8 rounded-[2rem]">
            <div className="flex justify-center mb-6">
              <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-4 py-1.5 rounded-full">
                ...
                <span className="text-[9px] font-extrabold tracking-[0.2em] uppercase text-emerald-400">
                  DEPOSIT FUNDS
                </span>
              </div>
            </div>
            ...
            <form onSubmit={handleAddMoney} className="space-y-6">
              <DepositQuickAmounts selected={amount} onSelect={setAmount} />

              <div className="space-y-2">
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">
                  Custom Amount (₹)
                </label>
                <div className="flex items-center bg-[#111726] border border-white/10 rounded-full px-5 py-3.5 focus-within:border-emerald-500/50 transition-all group">
                  <Banknote className="text-slate-500 group-focus-within:text-emerald-400 w-4 h-4 mr-3" />
                  <input
                    type="number"
                    value={amount}
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center bg-[#111726] border border-white/10 rounded-full px-5 py-3.5 focus-within:border-emerald-500/50 transition-all group">
                  <TrendingUp className="text-slate-500 group-focus-within:text-emerald-400 w-4 h-4 mr-3" />
                  <input
                    type="text"
                    value={description}
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-full py-4 px-6 font-extrabold text-xs uppercase tracking-widest transition-all duration-200 active:scale-[0.99] flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer shadow-lg shadow-emerald-950/50"
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <span>
                      Add ₹
                      {amount ? Number(amount).toLocaleString("en-IN") : "0"} to
                      Account
                    </span>
                  </>
                )}
          ...
          <div className="grid grid-cols-2 gap-4">
            {depositInfoCards.map((card) => (
              <div
                key={card.label}
                className="bg-[#0b0f19]/60 border border-white/5 p-4 rounded-2xl"
              >
                <p className="text-[9px] font-bold uppercase tracking-wider text-slate-500 mb-1">
                  {card.label}
                </p>
                <p className="text-xs font-black text-white">{card.desc}</p>
              </div>
            ))}
          </div>
        </main>
